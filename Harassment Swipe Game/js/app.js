...
function populateCardBack(face, s, swipeDir) {
  ...
  bannerSpan.textContent = isCorrect ? t('correct') : t('incorrect');
...
/* ============================================================
   ...
   ============================================================ */
const SWIPE_THRESHOLD = 100;  // px before swipe commits
...
function handleButtonSwipe(dir) {
  ...
  // Show stamp + tint at full intensity so the user sees feedback
  updateDragFeedback(
    wrapper, dir,
    dir === 'right' ? 200 : dir === 'left' ? -200 : 0,
    dir === 'down'  ? 200 : 0
  );

  setTimeout(() => {
    isCommitting = false;
    commitSwipe(wrapper, dir);
  }, 300);
}
...
function commitSwipe(wrapper, dir, dragDx = 0, dragDy = 0) {
  ...
  // Bring to front so animation doesn't clip behind other cards
  ...
  if (dragDx !== 0 || dragDy !== 0) {
    // ── Drag swipe: card has momentum, continue briefly then flip back ──
    ...
    // At momentum peak: trigger flip + glide back to center
    setTimeout(() => {
    ...
    // ── Button / keyboard: arc from center (like the walkthrough demo) ──
    wrapper.style.transition = '';
    wrapper.style.transform  = '';
    wrapper.classList.add(`arc-${dir}`);
    // Flip at arc peak (45% of 500ms ≈ 225ms)
    setTimeout(() => { flip.classList.add('flipped'); }, 220);
    // Remove arc class after animation finishes
    setTimeout(() => { wrapper.classList.remove(`arc-${dir}`); }, 500);
  }
}
